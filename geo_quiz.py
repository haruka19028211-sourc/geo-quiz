...
def load_data():
    ...
    with open("shikuchoson.csv", "r", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            data.append({"pref": row["pref"], "city": row["city"]})
    return data
...
if current < TOTAL_QUESTIONS:
    ...
    for i, a in enumerate(answers, start=1):
        ...
        if a["is_correct"]:
            st.write(f"{mark} 第{i}問　**{a['city']}** → {a['correct']}")
        else:
            st.write(f"{mark} 第{i}問　**{a['city']}** → あなた: {a['selected']} ／ 正解: {a['correct']}")
